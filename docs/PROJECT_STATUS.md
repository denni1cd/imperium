...
- **Stage 5 Gate 2E.1:** implemented in draft PR #14 and proven under simulated providers; merge review is pending.
...
- **Current gate:** review Gate 2E.1 before implementing explicit abandon/retry authorization or any complete live council.
...
| 5 — Codex provider and live slice | Gates 1 and 2 merged; Gate 2E.1 in draft | Terra-low provider proven; shared provider-authoritative lifecycle and durable budgeted attempts pass simulated tests; retry authorization and captured replay remain |
...
Draft PR #14 proves under replay and simulated providers that:
...
- no operation currently authorizes that lineage;
- providers reporting an automatic retry are rejected.

The clean draft head passes **161 Python tests** and the Stage 4 artifact workflow. CI made no Codex or live council calls.

## Remaining Before the First Live Council

Gate 2E still requires:

- an explicit operator workflow to abandon an ambiguous/failed attempt or authorize exactly one replacement attempt;
